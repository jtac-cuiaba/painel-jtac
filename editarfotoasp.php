          ...
          <div class="col-sm-offset-3 col-sm-4 col-xs-12">
                ...
                if(isset($_POST['w'])){
                  ...
                  if($crop){
                    if($_SESSION['temp_img'] != ''){
                      $link->query("UPDATE `aspirante` SET `foto` = '".$crop."' WHERE `Idaspirante` = ".$Idaspirante);
            ...
            <?php else:?>
              <form class="form-group" action="" method="post" enctype="multipart/form-data">
                ...
                <br>
                <input type="submit" class="btn btn-primary" name="upl_foto" value="Enviar Foto do Aspirante" />
                <a href="editaraspira.php?id=<?php print $aspirante['Idaspirante']; ?>" class="btn btn-warning">Voltar</a>
              </form>
            <?php endif;?>
          </div>
        </div>
